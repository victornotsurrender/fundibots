...
local function ConsiderQ()
	...
	then
		...
		if #enemies > 0 and bot:GetHealth() <= (0.2+(#enemies*0.1))*bot:GetMaxHealth() 
		then
	...
	-- if npcBot:GetActiveMode() == BOT_MODE_LANING or mutil.IsPushing(npcBot) or mutil.IsDefending(npcBot)  and mutil.AllowedToSpam(bot, nManaCost)
	-- then
...
local function ConsiderR()
	...
	then
		...
		do
			if ( bot:WasRecentlyDamagedByHero( npcEnemy, 2.0 )  and bot:GetHealth()/bot:GetMaxHealth() <= 0.35  ) 
			then
				local cpos = utils.GetTowardsFountainLocation( bot:GetLocation(), 0);
				bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
