...
local useTrapTeleportChannel = false;
...
function GetDesire()
	...
	elseif bot:GetUnitName() == "npc_dota_hero_tinker" then
		...
		if ( cAbility:IsInAbilityPhase() or ( cAbility:IsInAbilityPhase() and bot:IsUsingAbility() and bot:IsCastingAbility() ) ) or bot:IsChanneling()
		
		or bot:HasModifier('modifier_tinker_rearm') then
			return BOT_MODE_DESIRE_ABSOLUTE;
		end
	...
	elseif bot:GetUnitName() == "npc_dota_hero_tiny" then
		...
		if cAbility:IsInAbilityPhase() or bot:IsChanneling() then
			useTrapTeleportChannel = true;
			return BOT_MODE_DESIRE_ABSOLUTE;
		end	
		
		elseif bot:GetUnitName() == "npc_dota_hero_dawnbreaker" then
		if cAbility == nil then cAbility = bot:GetAbilityByName( "dawnbreaker_solar_guardian" ) end;
		if cAbility:IsInAbilityPhase() or bot:IsChanneling() then
			
			return BOT_MODE_DESIRE_ABSOLUTE;
		end
		elseif bot:GetUnitName() == "npc_dota_hero_crystal_maiden" then
		if cAbility == nil then cAbility = bot:GetAbilityByName( "crystal_maiden_freezing_field" ) end;
		if cAbility:IsInAbilityPhase() or bot:IsChanneling() then
	...
	if bot:GetUnitName() == "npc_dota_hero_abaddon" and bot:HasModifier("modifier_abaddon_borrowed_time") 
		then
		return BOT_MODE_DESIRE_ABSOLUTE;
	end	
	...
	if bot:GetUnitName() == "npc_dota_hero_pudge" then
		...
		local creeps = bot:GetNearbyCreeps(250, true);
		-- if cAbility == nil then cAbility = bot:GetAbilityByName( "pudge_rot" ) end;
		if (#enemies >= 1 ) and not bot:IsChanneling() and bot:HasModifier("modifier_pudge_rot")  
		then
		-- cAbility:GetToggleState()
			return BOT_MODE_DESIRE_ABSOLUTE;
		end	
	end
	
	if bot:GetUnitName() == "npc_dota_hero_storm_spirit" then
		local attackRange = bot:GetAttackRange();
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( attackRange, true, BOT_MODE_NONE );
		for _,npcEnemy in pairs(tableNearbyEnemyHeroes)
		do
	  if  bot:HasModifier("modifier_storm_spirit_overload") 
		then
		 if mutil.IsValidTarget(npcEnemy) and mutil.IsInRange(bot, npcEnemy, attackRange) 
		 and mutil.CanCastOnMagicImmune(npcEnemy)
		  and GetUnitToUnitDistance (bot,npcEnemy) <= attackRange
		 then
			return BOT_MODE_DESIRE_ABSOLUTE;
		end
	  end
	...
	if bot:GetUnitName() == "npc_dota_hero_lycan" and bot:HasModifier("modifier_lycan_shapeshift") 
...
function Think()
	...
	if bot:GetUnitName() == "npc_dota_hero_shadow_shaman" 
		...
		or  bot:IsChanneling()
		 or  (bot:GetUnitName() == "npc_dota_hero_drow_ranger" and bot:IsUsingAbility() and bot:IsCastingAbility() and  bot:IsChanneling() )
	then
		return;	
	elseif bot:GetUnitName() == "npc_dota_hero_magnataur" then
	...
	elseif bot:GetUnitName() == "npc_dota_hero_templar_assassin" then
		if useTrapTeleportChannel == true then
			return
		end
	...
	if (bot:GetUnitName() == "npc_dota_hero_drow_ranger" and bot:IsUsingAbility() and bot:IsCastingAbility() and  bot:IsChanneling() )
	then
		return;	
	...
	if bot:GetUnitName() == "npc_dota_hero_abaddon" and bot:HasModifier("modifier_abaddon_borrowed_time") then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( 1600, true, BOT_MODE_NONE );
		local tableNearbyAllyHeroes = bot:GetNearbyHeroes( 1600, false, BOT_MODE_NONE );
		local target = mutil.GetVulnerableWeakestUnit(true, true, 1600, bot);
		local loc = mutil.GetEscapeLoc();
		if target ~= nil and tableNearbyAllyHeroes ~= nil and tableNearbyEnemyHeroes ~= nil  and #tableNearbyAllyHeroes >= #tableNearbyEnemyHeroes then
		   ...
		   target = mutil.GetVulnerableWeakestUnit(false, true, 1600, bot);
		if target ~= nil then
			bot:Action_AttackUnit(target, true);
			return 
			else 
	...
	if bot:GetUnitName() == "npc_dota_hero_pudge" and bot:HasModifier("modifier_pudge_rot") and not bot:IsChanneling() then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( 1300, true, BOT_MODE_NONE );
		local tableNearbyAllyHeroes = bot:GetNearbyHeroes( 1300, false, BOT_MODE_NONE );
		local target = mutil.GetVulnerableWeakestUnit(true, true, 1300, bot);
		local loc = mutil.GetEscapeLoc();
		if target ~= nil and target:IsAlive() and target:CanBeSeen() and target:IsHero() 
			and tableNearbyAllyHeroes ~= nil and tableNearbyEnemyHeroes ~= nil  and #tableNearbyAllyHeroes >= #tableNearbyEnemyHeroes
		then
			bot:Action_AttackUnit(target, true);
			return
			elseif #tableNearbyEnemyHeroes == 0 then
				local target = mutil.GetVulnerableWeakestUnit(false, true, 1300, bot);
		if target ~= nil then
			bot:Action_AttackUnit(target, true);
			return 
			else 
	...
	if bot:GetUnitName() == "npc_dota_hero_storm_spirit" then
	  ...
	  if  bot:HasModifier("modifier_storm_spirit_overload")  
		then
		 ...
		 and GetUnitToUnitDistance (bot,npcEnemy) <= attackRange
		 then
			bot:Action_AttackUnit(npcEnemy, true);
			return 
		
		else
		bot:Action_ClearActions(true);
		-- local loc = mutil.GetEnemyFountain()
		-- bot:Action_MoveToLocation(loc);
		return
	end
	end
	...
	if bot:GetUnitName() == "npc_dota_hero_lycan" and bot:HasModifier("modifier_lycan_shapeshift") then
		-- local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( 1300, true, BOT_MODE_NONE );
		-- for _,npcEnemy in pairs(tableNearbyEnemyHeroes)
		-- do
	...
	-- then
		...
		local loc = mutil.GetEscapeLoc();
		if target ~= nil 
		and tableNearbyAllyHeroes ~= nil 
		and tableNearbyEnemyHeroes ~= nil  
		and #tableNearbyAllyHeroes >= #tableNearbyEnemyHeroes 
		and bot:GetHealth() / bot:GetMaxHealth() > 0.25
		then
			bot:Action_AttackUnit(target, true);
			return 
	...
	if bot:GetUnitName() == "npc_dota_hero_crystal_maiden" and bot:HasModifier("modifier_item_aghanims_shard") 
		then
		local nRadius  = cAbility:GetSpecialValueInt( "radius" )
		if bot:IsUsingAbility() and bot:IsCastingAbility() and bot:IsChanneling() and cAbility:IsInAbilityPhase() then
		local target = mutil.GetVulnerableWeakestUnit(true, true, 1600, bot);
		local loc = mutil.GetEscapeLoc();
			if mutil.IsGoingOnSomeone(npcBot) or mutil.IsInTeamFight(npcBot, 1200) then
			if target ~= nil then
			if mutil.IsInRange(bot,target,nRadius -100 ) == false then
