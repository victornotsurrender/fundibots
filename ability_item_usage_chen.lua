...
function ConsiderUnrefinedFireblast()
	...
	if ( not abilityUFB:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) then 
...
function ConsiderFireblast()
	...
	-- Make sure it's castable
	if ( not abilityFB:IsFullyCastable() or mutil.CanNotBeCast(npcBot)  or bot:HasModifier("modifier_item_aghanims_shard") == false ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
...
function ConsiderHolyPersuasion()
	...
	-- Make sure it's castable
	if ( not abilityHP:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
...
function ConsiderHandofGod()
	...
	-- Make sure it's castable
	if ( not abilityHoG:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) then 
