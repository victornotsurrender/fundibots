...
function ConsiderE()
	...
	-- Make sure it's castable
	if ( not abilityE:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
	...
	then
		...
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( nDmgRadius );
	...
	then
		...
		do
			if creep:GetHealth() <= nDmgRadius then
				return BOT_ACTION_DESIRE_MODERATE, npcBot;
		    end
        end
	end
	...
	local skThere, skLoc = mutil.IsSandKingThere(npcBot, nDmgRadius - 100, 2.0);
