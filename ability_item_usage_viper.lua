...
function TogglePoisonAttack()
	
	local currManaP = npcBot:GetMana() / npcBot:GetMaxMana();

	if ( npcTarget ~= nil and ( npcTarget:IsHero() or npcTarget:IsCreep()or npcTarget:IsTower() or npcTarget:GetUnitName() == "npc_dota_roshan" ) and mutil.CanCastOnNonMagicImmune(npcTarget) and currManaP > .25 ) 
	then
		if not abilities[1]:GetAutoCastState( ) then
			abilities[1]:ToggleAutoCast()
		end
	else 
		if  abilities[1]:GetAutoCastState( ) then
			abilities[1]:ToggleAutoCast()
		end
	end
...
function ConsiderR()
	...
	then
		local npcTarget = npcBot:GetAttackTarget();
		if ( mutil.IsRoshan(npcTarget) and mutil.CanCastOnMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nCastRange)  and mutil.IsDisabled(true, npcTarget) )
		then
			return BOT_ACTION_DESIRE_LOW, npcTarget;
		end
	end
