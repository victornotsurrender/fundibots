...
function AbilityUsageThink()
	...
	if abilityDZ == nil then abilityDZ = npcBot:GetAbilityByName( "terrorblade_demon_zeal" ); end
	if abilityTW == nil then abilityTW = npcBot:GetAbilityByName( "terrorblade_terror_wave" ); end
	...
	castDZDesire = ConsiderDemonZeal();
	castTWDesire = ConsiderTerrorWave();
	...
	if ( castDZDesire > 0 ) 
	then
		npcBot:Action_UseAbility( abilityDZ );
		return;
	end
	
	if ( castTWDesire > 0 ) 
	then
		npcBot:Action_UseAbility( abilityTW );
		return;
	end
...
function ConsiderDemonZeal()
	...
	-- Make sure it's castable
	if ( not abilityDZ:IsFullyCastable() or abilityDZ:IsHidden() or bot:HasModifier('modifier_item_aghanims_shard') == false ) then 
		return BOT_ACTION_DESIRE_NONE;
	end
	
	local nRadius = npcBot:GetAttackRange() + 200;
...
function ConsiderTerrorWave()
	...
	-- Make sure it's castable
	if ( not abilityTW:IsFullyCastable() or abilityTW:IsHidden()
	or npcBot:HasScepter()== false ) then 
		return BOT_ACTION_DESIRE_NONE;
	end
	
	local nRadius = abilityTW:GetSpecialValueInt( "scepter_radius" );
	...
	then
		local npcTarget = npcBot:GetTarget();
		if mutil.IsValidTarget(npcTarget) and mutil.IsInRange( npcTarget, npcBot, nRadius/2)
		then
			local cpos = utils.GetTowardsFountainLocation(npcTarget:GetLocation(), 0);
				bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
			return BOT_ACTION_DESIRE_MODERATE;
...
function ConsiderFireblast()
	...
	then
		...
		if ( sunderTarget ~= nil  )
		then
			local cpos = utils.GetTowardsFountainLocation(sunderTarget:GetLocation(), 0);
				bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
