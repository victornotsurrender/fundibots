...
function ConsiderSpirits()
	...
	then
		local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( nMaxRange + 200, true, BOT_MODE_NONE );
	...
	if npcBot:GetActiveMode() == BOT_MODE_LANING  or  mutil.IsPushing(npcBot) or  mutil.IsDefending(npcBot) and currManaP > 0.40
	then
	local laneCreeps = npcBot:GetNearbyLaneCreeps(nMaxRange + 200, true);
	   if ( lanecreeps~= nil and #lanecreeps >= 4  ) 
		then		
		for _,creep in pairs(laneCreeps)
		do
			if  creep ~= nil and GetUnitToUnitDistance( creep, npcBot ) < nMaxRange then
	...
	then
		local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes(  nMaxRange +200, true, BOT_MODE_NONE );
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( nMaxRange +200 );
...
function ConsiderSpiritsIn()
	...
	then
	local laneCreeps = npcBot:GetNearbyLaneCreeps(nMaxRange +200, true);
	   if ( lanecreeps~= nil and #lanecreeps >= 1  ) 
		then		
		for _,creep in pairs(laneCreeps)
		do
			if  creep ~= nil and GetUnitToUnitDistance( creep, npcBot ) < nMaxRange/2 then
				return BOT_ACTION_DESIRE_HIGH;
		    end
        end
	end
	...
	then
		...
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( nMaxRange +200 );
			if tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >= 1 then
				...
				if tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes == 0 and tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >=1
					then 
					if  neutral ~= nil and GetUnitToUnitDistance( neutral, npcBot ) < nMaxRange/2 then
				return BOT_ACTION_DESIRE_HIGH;
				end
			end
		end
	...
	then
		...
		do
			if ( npcBot:WasRecentlyDamagedByHero( npcEnemy, 2.0 ) ) 
			then
			if ( npcEnemy ~= nil and GetUnitToUnitDistance( npcEnemy, npcBot ) < nMaxRange /2 )
		    then
				return BOT_ACTION_DESIRE_MODERATE;
			end
		end
...
function ConsiderOverCharge()
	...
	local nManaCost = abilityOC:GetManaCost( );
	...
	if ( mutil.IsPushing(npcBot)  ) and mutil.AllowedToSpam(npcBot, nManaCost) 
	then
