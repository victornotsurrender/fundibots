...
function AbilityUsageThink()
	...
	if abilityQ == nil then abilityQ = npcBot:GetAbilityByName( "void_spirit_aether_remnant" ) end
	if abilityW == nil then abilityW = npcBot:GetAbilityByName( "void_spirit_dissimilate" ) end
	if abilityE == nil then abilityE = npcBot:GetAbilityByName( "void_spirit_resonant_pulse" ) end
...
function ConsiderQ()
	...
	local nCastRange = abilityQ:GetCastRange()-200;
	local nCastPoint = abilityQ:GetCastPoint( );
	local nManaCost  = abilityQ:GetManaCost( );
	local nDamage    = abilityQ:GetSpecialValueInt( 'impact_damage');
...
function ConsiderW()
	...
	-- Get some of its values
	local nRadius   = abilityW:GetSpecialValueInt( "first_ring_distance_offset" );
...
function ConsiderE()
	...
	then
		local npcTarget = npcBot:GetAttackTarget();
		if ( mutil.IsRoshan(npcTarget) and mutil.CanCastOnMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nRadius - 50)  )
		then
			return BOT_ACTION_DESIRE_LOW;
		end
	end
		...
		then
			...
			if targetEnemy ~= nil and targetAlly ~= nil and #targetEnemy >= #targetAlly then
				if npcTarget:GetMovementDirectionStability() < 1.0 then
					...
					bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
					-- return BOT_ACTION_DESIRE_HIGH, npcTarget:GetLocation();
					return BOT_ACTION_DESIRE_HIGH,npcBot:GetXUnitsTowardsLocation( npcTarget:GetLocation(), nCastRange )
				else
