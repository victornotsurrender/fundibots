...
local function ConsiderQ()
	...
	then
	   local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( nCastRange, true, BOT_MODE_NONE );
		...
		do
			...
			then
				return BOT_ACTION_DESIRE_HIGH, npcEnemy,1.0;
			end
		end
		...
		for _,u in pairs(Atowers) do
			...
			if #allies >= 0 then
				local cpos = utils.GetTowardsFountainLocation( npcEnemy:GetLocation(), 0);
					...
					return BOT_ACTION_DESIRE_MODERATE, npcEnemy,2.5;
				end
			end
		end
	end
	...
	then
		...
		-- local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( nCastRange );
			local creeps = bot:GetNearbyNeutralCreeps(nCastRange+nRadius);
		for i=1, #creeps do
			if creeps[i] ~= nil 
			...
			then	
				...
				if n_creeps >= 1 then
					...
					return BOT_ACTION_DESIRE_MODERATE, creeps[i], 2.5;
				end
			end
		end
	end
	...
	then
		...
		then
			return BOT_ACTION_DESIRE_LOW, npcTarget,2.5;
		end
	end
