...
function ConsiderNatureGrasp()
	...
	if npcBot:GetActiveMode() == BOT_MODE_FARM and npcBot:GetMana() / npcBot:GetMaxMana() > 0.45
	then
	...
	if ( mutil.IsDefending(npcBot) or mutil.IsPushing(npcBot) or bot:GetActiveMode() == BOT_MODE_LANING ) and npcBot:GetMana() / npcBot:GetMaxMana() > 0.45
	then
...
function ConsiderLeechSeed()
	...
	local nManaCost = abilityLS:GetManaCost();
	local currManaP = npcBot:GetMana() / npcBot:GetMaxMana();
	--------------------------------------
	-- Mode based usage
	--------------------------------------
	...
	if ( mutil.IsPushing(npcBot) or mutil.IsDefending(npcBot) or bot:GetActiveMode() == BOT_MODE_LANING ) and currManaP > 0.45
	then
		local lanecreeps = npcBot:GetNearbyLaneCreeps(nCastRange, true);
		
		if ( lanecreeps~= nil and #lanecreeps >= 8  ) 
		then
			return BOT_ACTION_DESIRE_LOW,lanecreeps[1];
		end
	end
...
function ConsiderOvergrowth()
	...
	local nManaCost = abilityOG:GetManaCost();
