...
function ItemUsageModule.ShouldTP(bot)
	...
	if  bot:GetHealth() < 150  and enemies == 0 and #enemyTowers == 0 and bot:DistanceFromFountain() > tpThreshold  then
	  -- local ancient = GetAncient(GetTeam());
		bot:ActionImmediate_Chat("Me retiro en tp!!!.", false)       
	...
	local ancient = GetAncient(GetTeam());
	local enemyHeroCloseToBase = ancient:GetNearbyHeroes(1600, true, BOT_MODE_NONE)
	
	
	if mode == BOT_MODE_RETREAT and enemies >= 3 then
	  local allies = bot:GetNearbyHeroes( 1600, false, BOT_MODE_ATTACK );
	  
	 if  #allies ==0 and bot:DistanceFromFountain() > 4500 and bot:HasModifier("modifier_dark_willow_shadow_realm_buff") then        
		tpLoc = mutil.GetTeamFountain()
	...
	if mode == BOT_MODE_RETREAT and enemies >= 1 then
	 if  bot:DistanceFromFountain() > 4500 and bot:HasModifier("modifier_dark_willow_shadow_realm_buff") and bot:GetHealth() < 0.25*bot:GetMaxHealth()
		then		
       	tpLoc = mutil.GetTeamFountain()	
	end
	end
	...
	if  bot:DistanceFromFountain() > tpThreshold and role.IsSupport(unitName) then
	 
	  local locationAoE = bot:FindAoELocation( true, false, ancient:GetLocation(), 0, 800, 0, 0 );
	  
	 if ( locationAoE.count >= 4 ) 
	 
	 and enemies == 0 then         
		bot:ActionImmediate_Chat("Defending the Ancient!!!.", false)       
			tpLoc = mutil.GetTeamFountain()
	...
	if  bot:DistanceFromFountain() > tpThreshold  then
	  local ancient = GetAncient(GetTeam());
	  --local tableNearbyEnemyHeroes = ancient:GetNearbyHeroes( 1600, true, BOT_MODE_NONE );
	  local locationAoE = bot:FindAoELocation( true, true, ancient:GetLocation(), 0, 1000, 0, 0 );
	  
	 if  ( locationAoE.count >= 1 )
	 and enemies == 0 then 
		-- local cpos = utils.GetTowardsFountainLocation(GetAncient(GetTeam()):GetLocation(), 0);
		-- bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
		bot:ActionImmediate_Chat("Defending the Ancient!!!.", false)       
			tpLoc = mutil.GetTeamFountain()
		elseif locationAoE.count >= 1 and bot:DistanceFromFountain() <= tpThreshold 
			and GetUnitToLocationDistance ( bot,ancient:GetLocation() ) > 1000
	...
	-- if ( enemyHeroCloseToBase ~=nil and #enemyHeroCloseToBase >= 1 ) 
		-- and enemies == 0 
	 -- then 
		
			-- bot:ActionImmediate_Chat("Defending the Ancient!!!.", false) 
			-- local cpos = mutil.GetCenterOfUnits(enemyHeroCloseToBase);
			-- bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
			-- tpLoc = mutil.GetTeamFountain()
		
	   -- end
	   -- -- if ( enemyHeroCloseToBase ~=nil and #enemyHeroCloseToBase >= 1 )
			-- -- and DotaTime() > lastPing + 3.0 	   
	   -- -- then
	   -- -- 
	   -- -- 
	   -- -- lastPing = DotaTime()
...
ItemUsageModule.Use['item_tango'] = function(item, bot, mode, extra_range)
	...
	if	bot:GetLevel() <= 12 and DotaTime() > 10 and role.CanBeSupport(bot:GetUnitName())
	...
	elseif bot:GetActiveMode() == BOT_MODE_LANING and role.CanBeSupport(bot:GetUnitName()) and DotaTime() > 10 and tCharge > 1 and DotaTime() > giveTime + 2.0 then
...
ItemUsageModule.Use['item_soul_ring'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsRetreating(bot)
		...
		do
			...
			then			
				if bot:GetHealth() - 150 > 0.35 * bot:GetMaxHealth() and bot:GetMana() < 0.5 * bot:GetMaxMana() 
					then
					local skillslot = {0,1,2,3,4,5};
						for i=1, #skillslot do
						local ability = bot:GetAbilityInSlot(skillslot[i]);
...
ItemUsageModule.Use['item_mask_of_madness'] = function(item, bot, mode, extra_range)
	...
	if  n_ability <= 1 and mutil.IsGoingOnSomeone(bot) then
		local target = bot:GetTarget();
		if  mutil.IsValidTarget(target) 
	...
	if bot:GetActiveMode() ~= BOT_MODE_FARM or mutil.IsPushing(bot)
	then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(1600,true,BOT_MODE_NONE);
		local target = bot:GetAttackTarget();
		if target ~= nil and not target:IsBuilding() and #tableNearbyEnemyHeroes == 0
		then
			return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
		end
	end	
	...
	then
		local tableNearbyEnemyHeroes = bot:GetNearbyHeroes(1600,true,BOT_MODE_NONE);
		local tableNearbyNeutrals = bot:GetNearbyNeutralCreeps ( 700);
		
		if tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >= 1 and #tableNearbyEnemyHeroes == 0
			then
			local target = bot:GetAttackTarget();
			
			if target ~= nil and not target:IsBuilding() and target ~= tableNearbyNeutrals[1]
				then
...
ItemUsageModule.Use['item_manta'] = function(item, bot, mode, extra_range)
	...
	if (( bot:IsSilenced() or bot:IsRooted()) and bot:HasModifier("modifier_item_mask_of_madness_berserk ") == false   ) 
	then
		return BOT_ACTION_DESIRE_ABSOLUTE, nil, 'no_target';
	end
...
ItemUsageModule.Use['item_meteor_hammer'] = function(item, bot, mode, extra_range)
	...
	elseif  mutil.IsInTeamFight(bot, 1200) then
		local locationAoE = bot:FindAoELocation( true, true, bot:GetLocation(), 600, 300, 2.5, 0 );
	...
	then
		...
		local tableNearbyEnemyCreeps = bot:GetNearbyCreeps( nCastRange, true );
		local locationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), 600, 300, 2.5, 0 );
		if ( locationAoE.count >= 4 and #tableNearbyEnemyCreeps >= 4  ) and GetUnitToLocationDistance (bot,locationAoE.targetloc) < nCastRange
		then
		-- local target = bot:GetAttackTarget();
		-- if target ~= nil and not target:IsBuilding() then
			-- return BOT_ACTION_DESIRE_LOW,target:GetLocation(), 'point';
		-- end
			return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc, 'point';
		end
	end	
	...
	local tableNearbyEnemyHeroes = bot:GetNearbyHeroes( nCastRange, true, BOT_MODE_NONE );
		local target = mutil.GetVulnerableWeakestUnit(true, true, nCastRange, bot);
		if target ~= nil and mutil.IsDisabled(true, target)
		then
	...
	for i = 1, #enemyPids do
		...
		if IsHeroAlive(enemyPids[i]) == true and info ~= nil then
			...
			if dInfo ~= nil 
				and dInfo.time_since_seen > 0.20 
				and dInfo.time_since_seen < 0.50 
				and GetUnitToLocationDistance(bot, dInfo.location) + 150 >  nCastRange 
			...
			then	
				...
				if IsLocationVisible(front_loc) == true and IsLocationPassable(front_loc) == true
				then
				-----------------------
				if (bot:GetActiveMode() == BOT_MODE_ROAM or bot:GetActiveMode() == BOT_MODE_TEAM_ROAM or bot:GetActiveMode() == BOT_MODE_GANK)
				  then
...
ItemUsageModule.Use['item_ethereal_blade'] = function(item, bot, mode, extra_range)
	...
	if mutil.IsGoingOnSomeone(bot) and role.IsNuker(unitName) 
	then
		local target = ItemUsageModule.GetNonDisabledStrongestEnemy(bot, nCastRange);
		if target ~= nil then
			return BOT_ACTION_DESIRE_ABSOLUTE, target, 'unit';
...
ItemUsageModule.Use['item_smoke_of_deceit'] = function(item, bot, mode, extra_range)
	...
	local tableNearbyEnemyHeroes=bot:GetNearbyHeroes(1100,true,BOT_MODE_NONE);
	local tableNearbyAlliedHeroes = bot:GetNearbyHeroes( 1000, true,BOT_MODE_NONE );
	...
	if   bot:GetActiveMode() == BOT_MODE_ROAM or
		 ...
		 bot:GetActiveMode() == BOT_MODE_NONE or
		 bot:GetActiveMode() == BOT_MODE_RUNE or
		 bot:GetActiveMode() == BOT_MODE_FARM or
		 bot:GetActiveMode() == BOT_MODE_LANING or
		 (bot:DistanceFromFountain () < 1000 and  bot:DistanceFromFountain () > 500) 
		 or bot:DistanceFromFountain () > 1000
		 
	 then
		if  tableNearbyAlliedHeroes ~= nil 
			and #tableNearbyAlliedHeroes >= 2
			and #tableNearbyEnemyHeroes == 0
			and bot:HasModifier("modifier_smoke_of_deceit") == false 
	...
	if   bot:GetActiveMode() == BOT_MODE_ROAM or
		 ...
		 bot:GetActiveMode() == BOT_MODE_NONE or
		 bot:GetActiveMode() == BOT_MODE_RUNE or
		 (bot:DistanceFromFountain () < 1000 or bot:DistanceFromFountain () > 500) or bot:DistanceFromFountain () > 1000
		 
	 then
		if  tableNearbyAlliedHeroes ~= nil 
			and #tableNearbyAlliedHeroes >= 2 and #tableNearbyEnemyHeroes == 0
			and bot:HasModifier("modifier_smoke_of_deceit") == false 
	...
	if   bot:GetActiveMode() == BOT_MODE_ROAM or
		 ...
		 bot:GetActiveMode() == BOT_MODE_NONE or
		 bot:GetActiveMode() == BOT_MODE_RUNE or
		 (bot:DistanceFromFountain () < 1000 or bot:DistanceFromFountain () > 500) 
		 or bot:DistanceFromFountain () > 1000
		 
	 then
		if  tableNearbyAlliedHeroes ~= nil 
			and #tableNearbyAlliedHeroes >= 0 and #tableNearbyEnemyHeroes == 0 and bot:GetHealth()/bot:GetMaxHealth() < 0.25
			and bot:HasModifier("modifier_smoke_of_deceit")==false 
