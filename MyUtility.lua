...
function U.CanNotBeCast(npcBot)
	if npcBot:IsRooted( ) or npcBot:IsStunned( )  or npcBot:IsNightmared() or npcBot:IsHexed( ) or npcBot:IsSilenced( ) 
     or npcBot:HasModifier("modifier_obsidian_destroyer_astral_imprisonment_prison")
	 or npcBot:HasModifier("modifier_shadow_demon_disruption")  or npcBot:HasModifier("modifier_doom_bringer_doom")
	 or U.IsTaunted2(npcBot) 
...
function U.IsDisabled2(npcBot)
	if npcBot:IsRooted( ) or npcBot:IsStunned( )  or npcBot:IsNightmared() or npcBot:IsHexed( ) or npcBot:IsSilenced( ) 
     or npcBot:HasModifier("modifier_obsidian_destroyer_astral_imprisonment_prison")
	 or npcBot:HasModifier("modifier_shadow_demon_disruption")  or npcBot:HasModifier("modifier_doom_bringer_doom")
	 or npcBot:HasModifier('modifier_bloodseeker_rupture') or npcBot:HasModifier("modifier_crystal_maiden_frostbite") 
...
function U.IsSlowed2(npcBot)
	local speedPlusBoots =  U.GetUpgradedSpeed2(npcBot);
	return npcBot:GetCurrentMovementSpeed() < speedPlusBoots;
end


function U.GetUpgradedSpeed2(npcBot)
	for i=0,5 do
		local item = npcBot:GetItemInSlot(i);
		if item ~= nil and listBoots[item:GetName()] ~= nil then
			return npcBot:GetBaseMovementSpeed()+listBoots[item:GetName()];
		end
	end
	return npcBot:GetBaseMovementSpeed();
end


function U.HasDisabledSkills(bot)
	return bot:HasModifier("modifier_viper_nethertoxin") 
	    or bot:HasModifier("modifier_silver_edge_debuff") 
	   
end



function U.CanBeDominatedCreeps2(name)
	return name == "npc_dota_neutral_granite_golem"
...
function U.GetVulnerableWeakestUnit2(bHero, bEnemy, nRadius, bot)
	local units = {};
	local weakest = nil;
	local weakestHP = 10000;
	if bHero then
...
function U.GetVulnerableWeakestUnit3(bHero, bEnemy, nRadius, bot)
	local units = {};
	local weakest = nil;
	local weakestHP = 1000;
	if bHero then
		units = bot:GetNearbyHeroes(nRadius, bEnemy, BOT_MODE_NONE);
	else
		units = bot:GetNearbyCreeps(nRadius, bEnemy);
...
function U.IsSandKingThere(bot, nCastRange, fTime)
	local enemies = bot:GetNearbyHeroes(1600, true, BOT_MODE_NONE);
	for _,enemy in pairs(enemies) do
		if enemy:GetUnitName() == "npc_dota_hero_sand_king" and enemy:HasModifier('modifier_sandking_sand_storm') or
			enemy:GetUnitName() == "npc_dota_hero_templar_assassin" and enemy:HasModifier('modifier_templar_assassin_meld') or
			enemy:GetUnitName() == "npc_dota_hero_clinkz" and enemy:HasModifier('modifier_clinkz_wind_walk') or
			enemy:GetUnitName() == "npc_dota_hero_mirana" and enemy:HasModifier('modifier_mirana_moonlight_shadow') or 
			enemy:GetUnitName() == "npc_dota_hero_riki" and enemy:HasModifier('modifier_riki_permanent_invisibility') or
			enemy:GetUnitName() == "npc_dota_hero_nyx_assassin" and enemy:HasModifier('modifier_nyx_assassin_vendetta') or
			enemy:GetUnitName() == "npc_dota_hero_bounty_hunter" and enemy:HasModifier('modifier_bounty_hunter_wind_walk') or
			enemy:GetUnitName() == "npc_dota_hero_invoker" and enemy:HasModifier('modifier_invoker_ghost_walk_self') or 
			enemy:GetUnitName() == "npc_dota_hero_weaver" and enemy:HasModifier('modifier_weaver_shukuchi')or 
			enemy:GetUnitName() == "npc_dota_hero_windrunner" and enemy:HasModifier('modifier_windrunner_windrun_invis') or
			enemy:GetUnitName() == "npc_dota_hero_phantom_assassin" and enemy:HasModifier('modifier_phantom_assassin_blur') or
			enemy:GetUnitName() == "npc_dota_hero_oracle" and enemy:HasModifier('modifier_oracle_false_promise') or
			enemy:GetUnitName() == "npc_dota_hero_dark_willow" and enemy:HasModifier('modifier_dark_willow_shadow_realm_buff') or
			
			
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_mirana_moonlight_shadow') or 
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_glimmer_cape') or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_rune_invis') or			
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_invisible') or 
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_invisibility_edge')or 
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_invisibility_edge_windwalk')or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_oracle_false_promise')or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_glimmer_cape_fade')or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_shadow_amulet')or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_shadow_amulet_fade')or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_silver_edge')or
			string.gsub(enemy:GetUnitName(), "npc_dota_hero_", "") and enemy:HasModifier('modifier_item_silver_edge_windwalk')
...
function U.IsSuspiciousIllusion2(npcEnemy)
	--TO DO Need to detect enemy hero's illusions better
	local bot = GetBot();
	--Detect allies's illusions
	if npcEnemy:IsIllusion() or npcEnemy:HasModifier('modifier_illusion') 
	   or npcEnemy:HasModifier('modifier_phantom_lancer_doppelwalk_illusion') or npcEnemy:HasModifier('modifier_phantom_lancer_juxtapose_illusion')
       or npcEnemy:HasModifier('modifier_darkseer_wallofreplica_illusion') or npcEnemy:HasModifier('modifier_terrorblade_conjureimage')	   
	then
		return true;
	else
	   --Detect replicate and wall of replica illusions
	    if GetGameMode() ~= GAMEMODE_MO then
			if npcEnemy:GetTeam() ~= bot:GetTeam() then
				local TeamMember = GetTeamPlayers(GetTeam());
				for i = 1, #TeamMember
				do
					local ally = GetTeamMember(i);
					if ally ~= nil and ally:GetUnitName() == npcEnemy:GetUnitName() then
						return true;
					end
				end
			end
...
function U.CanCastOnNonMagicImmune2(npcEnemy)
	return npcEnemy:CanBeSeen() and npcEnemy:IsAlive() and not npcEnemy:IsMagicImmune() and not npcEnemy:IsInvulnerable() and not U.IsSuspiciousIllusion2(npcEnemy) ;
end


function U.CanKillTarget2(npcEnemy, dmg, dmgType)
	return npcEnemy:GetActualIncomingDamage( dmg, dmgType ) >= npcEnemy:GetHealth(); 
end

function U.ShouldEscape2(npcBot)
	local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( 1000, true, BOT_MODE_NONE );
	if (npcBot:WasRecentlyDamagedByAnyHero(2.0) or npcBot:WasRecentlyDamagedByTower(2.0)) or ( npcBot:WasRecentlyDamagedByAnyHero(2.0) and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes >= 1  ) 
	then
		return true;
	end
end
...
function U.IsValidCreepTarget(target)
	return target ~= nil and target:IsAlive() and target:IsCreep() and not target:IsBuilding(); 
end



function U.GetStrongestUnit2(nRange, hUnit, bEnemy, bMagicImune, fTime)
	-- local units = hUnit:GetNearbyHeroes(nRange, bEnemy, BOT_MODE_NONE)
	local units = hUnit:GetNearbyCreeps(nRange, bEnemy)
	local strongest_unit = nil;
	local maxPower = 0;
	for i=1, #units do
		if U.IsValidTarget(units[i]) and
...
function U.IsInRange2(npcEnemy, npcBot, nCastRange)
	return GetUnitToUnitDistance( npcEnemy, npcBot ) <= nCastRange;
end

function U.IsValidTarget2(npcEnemy)
	return npcEnemy ~= nil and npcEnemy:IsAlive() and npcEnemy:IsHero() and not  npcEnemy:IsNull(); 
end
...
function U.IsValidBuilding( nTarget )
...
function U.CanNotUseAction( bot )
...
function U.GetCenterOfUnits( nUnits )
...
function U.IsValidHero( nTarget )
