...
function ConsiderVacuum()
	...
	if ( not abilityVC:IsFullyCastable() or abilityVC:GetLevel() < 2 or mutil.CanNotBeCast(npcBot) ) 
	then 
	...
	local nManaCost  = abilityVC:GetManaCost();
...
function ConsiderIonShell()
	...
	-- Make sure it's castable
	if ( not abilityIS:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
	...
	then
		local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( 1300, true, BOT_MODE_NONE );
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( 320 );
			if tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >= 1 then
				for _,neutral in pairs(tableNearbyNeutrals)
				do
				if neutral:CanBeSeen() and neutral:IsAlive() and tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes == 0 and tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >=1
...
function ConsiderSurge()
	...
	-- Make sure it's castable
	if ( not abilitySG:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
...
function ConsiderWallOfReplica()
	...
	-- Make sure it's castable
	if ( not abilityWR:IsFullyCastable() or mutil.CanNotBeCast(npcBot) ) 
	then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
	...
	then
		...
		for _,npcEnemy in pairs( tableNearbyEnemyHeroes )
		do
			if ( npcBot:WasRecentlyDamagedByHero( npcEnemy, 2.0 ) ) and GetUnitToUnitDistance (npcEnemy,npcBot) < 500
			then
