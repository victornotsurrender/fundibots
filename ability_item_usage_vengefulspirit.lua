...
function ConsiderQ()
	...
	local nDamage   = abilities[1]:GetSpecialValueInt("magic_missile_damage");
	...
	then
		...
		then
			...
			do
			if mutil.CanCastOnNonMagicImmune2(npcEnemy) and abilities[1]:GetLevel() > 1 then
...
function ConsiderW()
	...
	if mutils.IsRetreating(bot) and bot:WasRecentlyDamagedByAnyHero(2.0)
	then
		local target = mutils.GetVulnerableWeakestUnit(true, true, nCastRange-200, bot);
		if target ~= nil then
	...
	then
		local locationAoE = bot:FindAoELocation( true, false, bot:GetLocation(), nCastRange-200, nRadius, 0, 0 );
