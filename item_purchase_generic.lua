...
local ItemUsageModule = require("bots/ItemUsageUtility" );
...
if bot:IsHero() then
	purchase = require( "bots/builds/item_build_" .. string.gsub(GetBot():GetUnitName(), "npc_dota_hero_", ""));
end
...
if DotaTime() < 0 then
bot:ActionImmediate_Chat("Somos la IA Aurora, descarga nuestra ultima versión en el canal de Telegram: Bots para el dota2(t.me/botsdeldota2)",false);
end
...
bot.buildVessel = false;
bot.buildDesolator = false;
bot.buildMedallion = false;
bot.buildSolarCrest = false;

bot.buildOrbCorrosion = false;
bot.buildFalconBlade = false;
...
local function GeneralPurchase()
	...
	if ( bot:GetGold() >= cost ) then
		...
		if bot.SecretShop and courier ~= nil and GetCourierState(courier) == COURIER_STATE_IDLE and courier:DistanceFromSecretShop() < 250 then
...
local buyAghShard = false;
local addVeryLateGameItemStrengthBlink = false
local addVeryLateGameItemAgilityBlink = false
local addVeryLateGameItemIntelligenceBlink = false


local SellEveryEarlyItem = false
local addVeryLateGameItemBootsChange = false
local addVeryLateGameItemBoots = false
...
function ItemPurchaseThink()  
	...
	if bot.theRole == 'support' then
		...
		if DotaTime() < 0 and bot:GetGold() >= GetItemCost( "item_clarity" ) and items.HasItem(bot, "item_clarity") == false 
		then
			bot:ActionImmediate_PurchaseItem("item_clarity");	end
		if role['invisEnemyExist'] == true and buyBootsStatus == true and bot:GetGold() >= GetItemCost( "item_dust" ) 
		...
		if GetItemStockCount( "item_ward_observer" ) > 1 and  DotaTime() > 0   and bot:GetGold() >= GetItemCost( "item_ward_observer" ) 
			and items.GetEmptyInventoryAmount(bot) >= 3 and items.GetItemCharges(bot, "item_ward_observer") < 2 and items.GetItemCharges(bot, "item_ward_dispenser") ==0   and bot:GetCourierValue() == 0
		then
			bot:ActionImmediate_PurchaseItem("item_ward_observer"); 
		end
		------------------------------ My Scripts
		if ( DotaTime() > 10*60 and buyBootsStatus == true ) and bot:GetGold() >= GetItemCost( "item_smoke_of_deceit" ) 
			and GetItemStockCount( "item_smoke_of_deceit" ) > 1 and items.GetItemCharges(bot, "item_smoke_of_deceit") < 1 and items.GetEmptyInventoryAmount(bot) >= 4 and items.GetItemCharges(bot, "item_ward_observer") < 1 and bot:GetCourierValue() == 0
		then
			bot:ActionImmediate_PurchaseItem("item_smoke_of_deceit"); 
		end
		if GetItemStockCount( "item_ward_sentry" ) > 6 and ( DotaTime() < 0 or ( DotaTime() > 0 and buyBootsStatus == true ) ) and bot:GetGold() >= GetItemCost( "item_ward_sentry" ) 
			and items.GetEmptyInventoryAmount(bot) >= 3 and items.GetItemCharges(bot, "item_ward_sentry") < 2 and items.GetItemCharges(bot, "item_ward_dispenser") ==0 and bot:GetCourierValue() == 0
		then
	...
	if  ( GetGameMode() ~= 23 and DotaTime() > 20*60 and DotaTime() > fullInvCheck + 2.0 and ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 ) ) 
		or ( GetGameMode() == 23 and DotaTime() > 10*60 and DotaTime() > fullInvCheck + 2.0  )
		-- or (GetGameMode() ~= GAMEMODE_AP and DotaTime() > 5*60  and ItemUsageModule.IsInventoryFull(bot) and DotaTime() > fullInvCheck + 2.0 and ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 ))
		-- or (GetGameMode() == GAMEMODE_AP and DotaTime() > 5*60  and ItemUsageModule.IsInventoryFull(bot) and DotaTime() > fullInvCheck + 2.0 and ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 ))
		or (GetGameMode() ~= GAMEMODE_AP   and ItemUsageModule.IsInventoryFull(bot) and DotaTime() > fullInvCheck + 2.0 and ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 ))
		or (GetGameMode() == GAMEMODE_AP   and ItemUsageModule.IsInventoryFull(bot) and DotaTime() > fullInvCheck + 2.0 and ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 ))
		...
		if emptySlot < 2 then
			for i=1,#items['earlyGameItem'] do
				...
				if itemSlot >= 0 and itemSlot <= 8 then
					...
					elseif item == "item_fluffy_hat" then
						if bot.buildHoly == false  then
							slotToSell = itemSlot;
							break;
						end
...
-------------------------------------------------------------------
					elseif item == "item_fluffy_hat" then
						if bot.buildOrbCorrosion == false  then
							slotToSell = itemSlot;
							break;
						end
						
					elseif item == "item_fluffy_hat" then
						if bot.buildFalconBlade == false  then
							slotToSell = itemSlot;
							break;
						end
-------------------------------------------------------------------------
					...
					elseif item == "item_blight_stone" then
						if bot.buildMedallion == false  then
							slotToSell = itemSlot;
							break;
						end	
					
					elseif item == "item_medallion_of_courage" then
						if bot.buildSolarCrest == false  then
							slotToSell = itemSlot;
							break;
						end
					...
					elseif item == "item_falcon_blade" then
					local fb = bot:GetItemInSlot(itemSlot);
						if fb~=nil and #bot.itemToBuy <= 3 then
							slotToSell = itemSlot;
							break;
						end
					elseif item == "item_orb_of_corrosion" then
						local oc = bot:GetItemInSlot(itemSlot);
						if oc~=nil and #bot.itemToBuy <= 3 then
							slotToSell = itemSlot;
							break;
						end
	...
	if DotaTime() > 30*60 and ( items.HasItem( bot, "item_travel_boots") or items.HasItem( bot, "item_travel_boots_2")) and
	   ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 )
	then	
	...
	---buy aghanims shard
	if DotaTime() > 22*60 and bot:GetGold() >= 2400 and 
	    bot:HasModifier("modifier_item_aghanims_shard") == false 
	then
	...
	if #bot.itemToBuy == 0 and addVeryLateGameItemStrengthBlink == false and items.HasItem( bot, "item_blink") 
		and bot:GetPrimaryAttribute() == ATTRIBUTE_STRENGTH
		then
				 
		bot.itemToBuy = {"item_overwhelming_blink"}
		bot:ActionImmediate_Chat( "itemToBuy+1 item_overwhelming_blink", true )
		addVeryLateGameItemStrengthBlink = true		
	end	
	
	if #bot.itemToBuy == 0 and addVeryLateGameItemAgilityBlink == false and items.HasItem( bot, "item_blink") 
		and bot:GetPrimaryAttribute() == ATTRIBUTE_AGILITY
		then
				 
		bot.itemToBuy = {"item_swift_blink"}
		bot:ActionImmediate_Chat( "itemToBuy+1 item_swift_blink", true )
		addVeryLateGameItemAgilityBlink = true		
	end	
	
	if #bot.itemToBuy == 0 and addVeryLateGameItemIntelligenceBlink == false and items.HasItem( bot, "item_blink") 
		and bot:GetPrimaryAttribute() == ATTRIBUTE_INTELLECT
		then
				 
		bot.itemToBuy = {"item_arcane_blink"}
		bot:ActionImmediate_Chat( "itemToBuy+1 item_arcane_blink", true )
		addVeryLateGameItemIntelligenceBlink = true		
	end	
	...
	if #bot.itemToBuy == 0 and( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 )
		
			then
			for i=1,#items['earlyGameItem2']
			do
				local ItemSlot = bot:FindItemSlot(items['earlyGameItem2'][i]);
			if ItemSlot >= 0 then
	...
	--Fill purchase table with super late game item_travel_boots_2
	if #bot.itemToBuy == 0 and addVeryLateGameItemBoots == false 
	then
		if items.HasItem(bot, 'item_guardian_greaves') == false
		and (items.HasItem( bot, "boots") or items.HasItem( bot, "item_phase_boots") 
		or items.HasItem( bot, "item_power_treads") or items.HasItem( bot, "item_tranquil_boots") or items.HasItem( bot, "item_arcane_boots")  )
		-- and not bot:GetUnitName() == "npc_dota_hero_keeper_of_the_light"
		then		
		bot.itemToBuy = {'item_travel_boots_2'}	
		bot:ActionImmediate_Chat( "itemToBuy+1 item_travel_boots_2 ", true )
		addVeryLateGameItemBoots = true	
	end
	end
	
	
	
	if  addVeryLateGameItemBootsChange == false 
	then
		if  items.HasItem( bot, "item_tranquil_boots") and items.HasItem( bot, "item_arcane_boots")  
		-- and not bot:GetUnitName() == "npc_dota_hero_keeper_of_the_light"
		then		
		-- bot.itemToBuy = {'item_travel_boots_2'}	
		-- bot:ActionImmediate_Chat( "itemToBuy+1 item_travel_boots_2 ", true )
		addVeryLateGameItemBootsChange = true	
	end
	end
		----------
	
	
	if ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 )  and addVeryLateGameItemBoots == true 
		and  items.HasItem( bot, "item_travel_boots")
		then
		for i=1,#items['earlyBoots']
			do
				local bootsSlot = bot:FindItemSlot(items['earlyBoots'][i]);
	...
	if ( bot:DistanceFromFountain() <= 100 or bot:DistanceFromSecretShop() < 250 )  and addVeryLateGameItemBootsChange == true 
		and  items.HasItem( bot, "item_tranquil_boots") and items.HasItem( bot, "item_arcane_boots")
		then
		for i=1,#items['earlyBoots2']
			do
				local bootsSlot = bot:FindItemSlot(items['earlyBoots2'][i]);
			if bootsSlot >= 0 then
				bot:ActionImmediate_SellItem(bot:GetItemInSlot(bootsSlot))	
