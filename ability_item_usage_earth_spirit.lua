...
function ConsiderQ()
	...
	then
		...
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps(800 ) 
			if tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >= 1 then
				...
				and mutil.IsInRange(neutral, npcBot, nUnitCR + 200)
			then
				local loc = mutil.GetCorrectLoc(neutral, GetUnitToUnitDistance(npcBot, neutral)/nSpeed)
				if stoneNearby then
					return BOT_ACTION_DESIRE_HIGH, loc, false, true;						
				elseif nStone >= 1 then
					return BOT_ACTION_DESIRE_HIGH, loc, true, false;
				-- elseif nStone < 1 then
					-- return BOT_ACTION_DESIRE_LOW, neutral, false, false;
				elseif nStone < 1 then
			-- local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( nUnitCR+200, true, BOT_MODE_NONE );
			local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps(nUnitCR+200 ) 
			local target = mutil.GetClosestUnit(tableNearbyNeutrals)
			if target ~= nil then
				return BOT_ACTION_DESIRE_HIGH, target, false, false; 
				end
			end
	...
	then
		local lanecreeps = npcBot:GetNearbyLaneCreeps(nStoneCR/2, true);
		local locationAoE = npcBot:FindAoELocation( true, false, npcBot:GetLocation(), nStoneCR, nRadius, nCastPoint, 0 );
		...
		if stoneNearby then
			...
			elseif nStone < 1 then
			local creeps = npcBot:GetNearbyLaneCreeps( nUnitCR+200, true);
			local target = mutil.GetClosestUnit(creeps)
			if target ~= nil then
				return BOT_ACTION_DESIRE_HIGH, target, false, false; 
			end
		end
	...
	local units = npcBot:GetNearbyHeroes( nUnitCR+200, false, BOT_MODE_NONE );
	for _,myFriend in pairs(units) 
	do
		if  (myFriend:GetUnitName() ~= npcBot:GetUnitName() and mutil.IsRetreating(myFriend) and
			...
			and myFriend:GetHealth() < 0.35*myFriend:GetMaxHealth()) or myFriend:HasModifier("modifier_earthspirit_petrify")
		then			
			if nStone < 1 then
			-- local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( nUnitCR+200, true, BOT_MODE_NONE );
			local target = mutil.GetClosestUnit(units)
			if target ~= nil and npcBot:IsFacingUnit(GetAncient(GetTeam()), 15) and npcBot:IsFacingUnit(myFriend, 15) then
				return BOT_ACTION_DESIRE_HIGH, target, false, false; 
	...
	then
		...
		then
			...
			elseif nStone >= 1 then
				return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc, true, false;
			-- elseif nStone < 1 then
				-- return BOT_ACTION_DESIRE_LOW, locationAoE.targetloc, false, false;
			elseif nStone < 1 then
			local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( nUnitCR+200, true, BOT_MODE_NONE );
			local target = mutil.GetClosestUnit(tableNearbyEnemyHeroes)
			if target ~= nil then
	...
	then
		...
		then
			...
			then
				local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( nUnitCR+200, true, BOT_MODE_NONE );
				local target = mutil.GetClosestUnit(tableNearbyEnemyHeroes)
			if target ~= nil and npcBot:IsFacingUnit(GetAncient(GetTeam()), 5) and npcBot:IsFacingUnit(target, 5)
			then
			local cpos = utils.GetTowardsFountainLocation(target:GetLocation(), 0);
					bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
				return BOT_ACTION_DESIRE_HIGH, target, false, false; 
...
function ConsiderW()
	...
	then
		local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( 1300, true, BOT_MODE_NONE );
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( nUnitCR+250 );
			if tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >= 1 then
				for _,neutral in pairs(tableNearbyNeutrals)
				do
				if tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes == 0 and tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >=1
					then 
					return BOT_ACTION_DESIRE_MODERATE,neutral:GetLocation(),false;
				end
			end
		end
	end
	...
	then
		...
		if nStone >= 1 and  mutil.IsRoshan(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nStoneCR + 200) 
		then			
		...
		elseif nStone < 1 and mutil.IsRoshan(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nUnitCR / 2)  then
			local loc = mutil.GetCorrectLoc(npcTarget, GetUnitToUnitDistance(npcBot, target)/nSpeed)
			return BOT_ACTION_DESIRE_HIGH, loc, false;
		end
	end
	...
	then
		local npcTarget = npcBot:GetTarget();
		if nStone >= 1 and mutil.IsValidTarget(npcTarget) and mutil.CanCastOnNonMagicImmune(npcTarget) and mutil.IsInRange(npcTarget, npcBot, nStoneCR + 200) and not mutil.IsSuspiciousIllusion(npcTarget)
		then
		...
		elseif nStone < 1 and mutil.IsValidTarget(npcTarget)
			and mutil.CanCastOnNonMagicImmune(npcTarget) 
			and mutil.IsInRange(npcTarget, npcBot, nUnitCR / 2) 
		then
			local loc = mutil.GetCorrectLoc(npcTarget, GetUnitToUnitDistance(npcBot, target)/nSpeed)
			local cpos = utils.GetTowardsFountainLocation(npcTarget:GetLocation(), 0);
					bot:ActionImmediate_Ping( cpos.x,  cpos.y, true)
	...
	if skThere and nStone >= 1 then
		return BOT_ACTION_DESIRE_MODERATE, skLoc, true;
	end
...
function ConsiderF()
	-- Make sure it's castable
	if  not abilityF:IsFullyCastable()  or ( not npcBot:HasScepter()  or not npcBot:HasModifier("modifier_item_ultimate_scepter_consumed") ) then 
		return BOT_ACTION_DESIRE_NONE, 0;
	end
	...
	-- If we're seriously retreating, see if we can land a stun on someone who's damaged us recently
	if mutil.IsRetreating(npcBot) and (npcBot:HasScepter() == true or npcBot:HasModifier("modifier_item_ultimate_scepter_consumed") )
	then
		
		local tableNearbyEnemyHeroes = npcBot:GetNearbyHeroes( 1200, true, BOT_MODE_NONE );
		for _,npcEnemy in pairs( tableNearbyEnemyHeroes )
		do
			if ( npcBot:WasRecentlyDamagedByHero( npcEnemy, 2.0 ) and mutil.IsInRange(npcEnemy, npcBot, nCastRange)   ) 
			then
	...
	local units = npcBot:GetNearbyHeroes( 1200, false, BOT_MODE_NONE );
	for _,myFriend in pairs(units) 
	do
		if  myFriend:GetUnitName() ~= npcBot:GetUnitName() and mutil.IsRetreating(myFriend) and
			myFriend:WasRecentlyDamagedByAnyHero(2.0) and mutil.CanCastOnNonMagicImmune(myFriend)
			and myFriend:GetHealth() < 0.35*myFriend:GetMaxHealth() and mutil.IsInRange(myFriend, npcBot, nCastRange) and (npcBot:HasScepter() == true or npcBot:HasModifier("modifier_item_ultimate_scepter_consumed") )
		then		
	...
	-- If we're going after someone
	if mutil.IsGoingOnSomeone(npcBot) and (npcBot:HasScepter() == true or npcBot:HasModifier("modifier_item_ultimate_scepter_consumed") )
	then
		local npcTarget = npcBot:GetTarget();
