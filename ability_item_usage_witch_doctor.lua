...
local function ConsiderQ()
	...
	then
		...
		-- local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( nCastRange );
			local creeps = bot:GetNearbyNeutralCreeps(nCastRange);
		for i=1, #creeps do
			if creeps[i] ~= nil 
...
local function ConsiderD()
	...
	-- if ( not abilityD:IsFullyCastable() or abilityD:IsHidden() or bot:HasModifier('modifier_item_aghanims_shard') == false ) 
	-- then 
		-- return BOT_ACTION_DESIRE_NONE, 0;
	-- end
	if  mutils.CanBeCast(abilities[5]) == false 
	...
	then
		...
		if mutils.IsValidTarget(target) 
			and mutils.CanCastOnNonMagicImmune(target) 
			and mutils.IsInRange(bot, target, 800) 
...
function AbilityUsageThink()
	...
	-- if abilityD == nil then abilityD = npcBot:GetAbilityByName( "witch_doctor_voodoo_switcheroo" ) end
