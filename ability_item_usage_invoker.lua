...
function AbilityUsageThink()
 ...
 if ( GetGameState() ~= GAME_STATE_GAME_IN_PROGRESS and GetGameState() ~= GAME_STATE_PRE_GAME ) then return false end
	...
	if mutil.IsRetreating(bot) and bot:HasModifier('modifier_invoker_ghost_walk_self') then return end
...
function ConsiderTornado(bot, nearbyEnemyHeroes)
	...
	local nRadius = abilityTO:GetSpecialValueInt("area_of_effect")
    ...
    for _, npcEnemy in pairs( nearbyEnemyHeroes ) do
        if npcEnemy:IsChanneling() and CanCastTornadoOnTarget(npcEnemy) and GetUnitToUnitDistance (npcBot,npcEnemy) < 1600 
		then
    ...
    if IsRetreating(bot) then
        for _,npcEnemy in pairs( nearbyEnemyHeroes ) do
            if bot:WasRecentlyDamagedByHero( npcEnemy, 2.0 ) and GetUnitToUnitDistance( bot, npcEnemy ) <= nDistance 
			and CanCastTornadoOnTarget(npcEnemy) and GetUnitToUnitDistance (npcBot,npcEnemy) < 1600
			then
	...
	then
		...
		local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( 1300 );
			if tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >= 1 and mutil.IsInRange(neutral,npcBot, 1300) then
				for _,neutral in pairs(tableNearbyNeutrals)
				do
				if tableNearbyEnemyHeroes ~= nil and #tableNearbyEnemyHeroes == 0 and tableNearbyNeutrals ~= nil and #tableNearbyNeutrals >=3
					then 
					local dist = GetUnitToUnitDistance( neutral, npcBot )
					if dist < (nDistance - 300) then					
					return BOT_ACTION_DESIRE_MODERATE,neutral:GetExtrapolatedLocation( dist/nSpeed );
				end
			end
		end
	end
	...
	then
        ...
        if dist < (nDistance - 300) then
			return BOT_ACTION_DESIRE_LOW, npcTarget:GetExtrapolatedLocation( dist/nSpeed );
		end
		end
	end
	
	
	local skThere, skLoc = mutil.IsSandKingThere(npcBot, nCastRange/2, 2.0);
...
function ConsiderChaosMeteor(bot, nearbyEnemyHeroes)
	...
	local currManaP = npcBot:GetMana() / npcBot:GetMaxMana();
	local nManaCost = abilityCM:GetManaCost()
...
function ConsiderGhostWalk(bot, nearbyEnemyHeroes)
    ...
    if IsRetreating(bot) then
        for _, npcEnemy in pairs( nearbyEnemyHeroes ) do
			...
			-- and GetUnitToUnitDistance( npcEnemy, bot ) < 600 
			then
